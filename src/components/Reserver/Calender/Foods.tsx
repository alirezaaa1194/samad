...
function Foods({ groupedFoods, weekOffset }: { groupedFoods: groupFoodType; weekOffset: number }) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-2 items-center p-2">
      ...
      {days.map((day, j) => (
        <div className="flex flex-col gap-2" key={day.key}>
          {groupedFoods
            ...
            .foods.map((food: foodType, i: number) => (
              <div key={food.id} className={`flex flex-col w-full bg-[#f2f2f2] p-2.5 rounded-lg border-2 ${user?.reserved_foods?.find((userFood) => userFood.food === food.id) ? "border-secondary" : "border-transparent"}`}>
                ...
                <span className="text-center text-black/90 font-DanaDemiBold text-sm mt-4">
                  {food.predefined_food?.name || food.custom_name}
                </span>
                <Button
                  onPress={() => {
                    setMainFood(food);
